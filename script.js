...
async function loadData(){
  let csvText = "";
  if(typeof window.DATA_CSV_URL === "string" && window.DATA_CSV_URL.trim()!==""){
    try{
      csvText = await fetchText(window.DATA_CSV_URL.trim());
    }catch(e){
      console.warn("Live CSV failed, falling back to local data.csv:", e);
      csvText = await fetchText("data.csv");
    }
  }else{
    csvText = await fetchText("data.csv");
  }
